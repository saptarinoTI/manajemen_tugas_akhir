    ...
    <div class="card">
        @if (!$seminar)
            <div class="card-header d-flex justify-content-end">
                <a href="{{ route('seminar-hasil.create') }}" class="btn btn-sm btn-primary">Daftarkan Seminar
                    Hasil</a>
            </div>
        @endif
        <div class="card-body">
                    ...
                    <tbody>
                        @if ($seminar)
                            <p class="fw-bold text-xs d-flex justify-content-end" style="letter-spacing: 1px">Submit :
                                {{ date('d F Y', strtotime($seminar->created_at)) }}</p>
                            <tr>
                                <td>
                                ...
                                <td class="text-center">
                                    <a href="#semhasDetail"
                                        data-remote="{{ route('seminar-hasil.show', $seminar->mahasiswa->nim) }}"
                                        data-bs-toggle="modal" data-bs-target="#semhasDetail"
                                        data-title="{{ $seminar->mahasiswa->nim }} - {{ strtoupper($seminar->mahasiswa->nama) }}"
                                        class="my-1 btn btn-sm btn-info">
                                        <i class="bi bi-eye-fill"></i>
                                    </a>
                                    @if ($seminar->status != 'diterima')
                                        <a href="{{ route('seminar-hasil.edit', $seminar->id) }}"
            ...
            @if ($seminar)
                <span class="fw-bold text-xs d-flex justify-content-end" style="letter-spacing: 1px">Last Update :
                    {{ date('d F Y', strtotime($seminar->updated_at)) }}</span>
            @endif
        </div>
    </div>
