        ...
        <div class="card-body">
                    ...
                    <tbody>
                        @if ($pendadaran)
                            <p class="fw-bold text-xs d-flex justify-content-end" style="letter-spacing: 1px">Submit :
                                {{ date('d F Y', strtotime($pendadaran->created_at)) }}</p>
                            <tr>
                                <td>
                                    @if ($pendadaran->status == 'diterima')
                                        ...
                                        @elseif ($pendadaran->status == "ditolak")
                                            ...
                                            @else
                                                <span class="badge bg-info">Dikirm
                                    @endif
                                    </span>
                                </td>
                                <td>{{ ucwords($pendadaran->keterangan) }}</td>
                                <td class="text-center">
                                    <a href="#pendadaranDetail"
                                        data-remote="{{ route('pendadaran.show', $pendadaran->id) }}"
                                        data-bs-toggle="modal" data-bs-target="#pendadaranDetail"
                                        data-title="{{ $pendadaran->mahasiswa->nim }} - {{ strtoupper($pendadaran->mahasiswa->nama) }}"
                                        class="btn btn-sm btn-info my-1">
                                        <i class="bi bi-eye-fill"></i>
                                    </a>
                                    @if ($pendadaran->status != 'diterima')
                                        <a href="{{ route('pendadaran.edit', $pendadaran->id) }}"
            ...
            @if ($pendadaran)
                <span class="fw-bold text-xs d-flex justify-content-end" style="letter-spacing: 1px">Last Update :
                    {{ date('d F Y', strtotime($pendadaran->updated_at)) }}</span>
            @endif
        </div>
    </div>
